             ...
             <div class="card">
                        <div class="card-body">
                            ...
                            <ul class="nav nav-tabs mb-3  col-md-12" role="tablist">
                                <li class="nav-item">
                                    <a class="nav-link"  href="orders">Processing</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link active"  href="orders-transit" >In Transit</a>
                                </li>
                                <li class="nav-item">
                  ...
                  <?php 
                      ...
                      $results = $conn->query("SELECT customers.fname, customers.lname, customers.address,
                                                ...
                                                (SELECT COUNT(*) FROM orderdetails WHERE orderdetails.order_number = orders.ordernumber) AS orderdetailsQuantity FROM orders LEFT JOIN customers on customers.customerid = orders.customer_id
                                                  WHERE orders.status = 'Transit' ORDER BY orders.order_id DESC");
                      $i = 1;
                      while ($rows = $results->fetch_assoc()) {
                       ...
                       ?>
                        <tr class="delete_mem<?php echo $id ?>" id="<?php echo $id ?>" >
                            ...
                            <td>
                              <div class="dropdown ml-1">
                                  ...
                                  <div class="dropdown-menu dropdown-menu-left">
                                    ...
                                    <a href="javascript:void(0);" class="dropdown-item" data-toggle="tooltip" data-role="next" data-id="<?php echo $id; ?>" id="<?php echo $id; ?>"><i class="fa fa-arrow-right" aria-hidden="true"></i> Out for Delivery</a>
                                    <a href="javascript:void(0);" class="dropdown-item" data-toggle="tooltip" data-role="back" data-id="<?php echo $id; ?>" id="<?php echo $id; ?>"><i class="fa fa-arrow-left" aria-hidden="true"></i> Processing</a>
                            ...
                            <td><span class="badge bg-info text-white"><?php echo ucwords($rows['status']); ?></span></td>
                        </tr>
                    <?php $i++; }
                   ?>
...
<script type="text/javascript">
    $(document).ready(function() {
      // alert('Ture');
      //  append values in input fields
      $(document).on('click','a[data-role=back]',function(){
            var id = $(this).attr("id");
            if (confirm("Move to Processing step?")) {
                $.ajax({
                    type: "POST",
                    url: "orders-pending-move.php",
                    data: ({
                        id: id
                    }),
                    cache: false,
...
<script type="text/javascript">
    $(document).ready(function() {
      ...
      $(document).on('click','a[data-role=next]',function(){
            var id = $(this).attr("id");
            if (confirm("Move to Out for delivery?")) {
                $.ajax({
                    type: "POST",
                    url: "orders-pending-move-next.php",
                    data: ({
                        id: id
                    }),
                    cache: false,
